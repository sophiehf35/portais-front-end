function carregaListaDeArtigosSubcategoria(config, slugDaPagina) {
    let paginaAtual = 1;
    ...
    // Se não encontrou categoria pai, não carrega nada
    if (!categoriaPai) {
        console.error('Subcategoria não encontrada ou categoria pai inválida');
        return;
    }
    ...
    fetch('/configuracao/json/artigos.json')
        ...
        .then(responseData => {
            ...
            exibirArtigosNaPagina(paginaAtual, slugDaPagina, categoriaPai);
            exibirPaginacao(slugDaPagina, categoriaPai);

            // Carrega tabela de artigos na sidebar
            carregarTabelaArtigosSubcategoria(config, data, slugDaPagina, categoriaPai);
        })
        .catch(error => {
            console.error('Erro ao buscar dados:', error);
        });

    function exibirArtigosNaPagina(pagina, slugSubcategoria, slugCategoriaPai) {
        const startIndex = (pagina - 1) * numeroArtigosPorPagina;
        const endIndex = startIndex + numeroArtigosPorPagina;
        
        // Filtra artigos pela subcategoria E pela categoria pai
        const artigosDaPagina = data.filter(item => 
            item.slug_categoria === slugCategoriaPai && 
            item.slug_subcategoria === slugSubcategoria
        ...
        artigosDaPagina.forEach(item => {
            ...
            linkImagem.href = (config.diretorio_blog === 'home' ? '/' : `/${config.diretorio_blog}/`) + slugCategoriaPai + '/' + slugSubcategoria + '/' + item.slug;
            ...
            tituloLink.href = (config.diretorio_blog === 'home' ? '/' : `/${config.diretorio_blog}/`) + slugCategoriaPai + '/' + slugSubcategoria + '/' + item.slug;
    ...
    function exibirPaginacao(slugSubcategoria, slugCategoriaPai) {
        const artigosDaPagina = data.filter(item => 
            item.slug_categoria === slugCategoriaPai && 
            item.slug_subcategoria === slugSubcategoria
        );
        ...
        if (totalPages > 1) {
            ...
            if (paginaAtual > 1) {
                ...
                prevPageLink.href = paginaAtual === 2 ? 
                    `${config.diretorio_blog === 'home' ? '/' : `/${config.diretorio_blog}/`}${slugCategoriaPai}/${slugSubcategoria}/` : 
            ...
            if (startPage > 1) {
                ...
                firstPageLink.href = `${config.diretorio_blog === 'home' ? '/' : `/${config.diretorio_blog}/`}${slugCategoriaPai}/${slugSubcategoria}/`;
...
function carregaConteudoDestaqueSubcategoria(config) {
...
function carregarTabelaArtigosSubcategoria(config, artigos, slugSubcategoria, slugCategoriaPai) {
    ...
    new gridjs.Grid({
        ...
        data: artigosDaPagina.map((artigo, index) => ({
            id: index + 1,
            titulo: gridjs.html(
                `<a href='${(config.diretorio_blog === 'home' ? '/' : `/${config.diretorio_blog}/`)}${slugCategoriaPai}/${slugSubcategoria}/${artigo.slug}'>${artigo.titulo_breadcumb}</a>`
            )
        })),
        className: {
